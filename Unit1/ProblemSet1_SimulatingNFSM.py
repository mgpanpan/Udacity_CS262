...
## solution1 : using reduce
def nfsmsim(string, current, edges, accepting):
    if string == "":
        return current in accepting
    else:
        trans = (current, string[0])
        if trans in edges:
            all_out = map(lambda x: nfsmsim(string[1:], x, edges, accepting), edges[trans])
            return functools.reduce(lambda x, y: x or y, all_out, False)
        else:
            return False

# solution 2 : iteration, actually same as solution 1
def nfsmsim(string, current, edges, accepting):
    if string == "":
        return current in accepting
    else:
...
def nfsmsim(string, current, edges, accepting):
    if string == "":
        ...
        trans = (current, string[0])
        if trans in edges:
            for nextstate in edges[trans]:
                if (nfsmsim(string[1:], nextstate, edges, accepting)):
                    return True
        return False
