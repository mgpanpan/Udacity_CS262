...
# the p_tagname and p_tagnameend procedure can examine whether the tags are matched.
def p_tagname(p):
    'tagname : WORD'
    global tag_stack
    tag_stack = [(p[1], p.lineno(1))] + tag_stack    # push
    p[0] = p[1]
...
def p_tagnameend(p):
    ...
    if (tag_stack[0])[0] != p[1]:
        print("HTML Syntax Error: <" + tag_stack[0][0] + "> on line " +
              str(tag_stack[0][1]) + " closed by </" + str(p[1]) + "> on line "
              + str(p.lineno(1)))
        exit(1)
    tag_stack = tag_stack[1:]    # pop
    p[0] = p[1]

def p_element_tag_emtpy(p):
    # <br />
    'element : LANGLE tagname tag_arguments SLASHRANGLE'
    global tag_stack
    tag_stack = tag_stack[1:]    # !important
