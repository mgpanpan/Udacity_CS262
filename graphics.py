...
#### modified by pmg,
# change \underline{ to \\underline{
# add pdflatex path, cause pdflatex is not added to my environment variable
# omit the converting processing. regard the pdf out as the html page output.
...
def word(x):
    ...
    for i in range(len(x)):
        if x[i] == '_':
            outfile.write("\_")
        elif x[i] == '$':    # $ is a special character for input inline math equation
            outfile.write("\$")
        elif x[i] != '\\':
            outfile.write(x[i])
    outfile.write(" ")
...
pdflatex_path = "D:\\texlive\\2013\\bin\\win32\\"
...
""")
        ...
        cmd = pdflatex_path + "pdflatex " + output_latex_filename + ".tex"
